...
export const ProductionStudio: React.FC<ProductionStudioProps> = ({ 
  ...
  const allClipsDone = !isGenerating && storyboard.length > 0 && generatedMedia.every(v => v.status === 'complete');
  ...
  return (
    <div className="space-y-8">
      ...
      <SectionCard>
        <div className="p-8">
            <div>
                <h2 className="text-3xl font-bold font-display text-[var(--accent-primary)] mb-1">{songDetails.title}</h2>
                <p className="text-[var(--text-secondary)] mb-4">{songDetails.artist || 'Unknown Artist'}</p>
            </div>
            {isGenerating && <p className="text-[var(--accent-primary)] animate-pulse">{generationStatusMessage}</p>}
            {!isGenerating && allClipsDone && !finalVideoUrl && <p className="text-green-400">All assets are ready! You can now create the final video.</p>}
            {!isGenerating && finalVideoUrl && <p className="text-green-400">Production Complete! Preview and download your final music video below.</p>}
        </div>
      </SectionCard>
      ...
      {allClipsDone && (
        <SectionCard>
            <div className="p-8 text-center">
                {finalVideoUrl ? (
                    <>
                        <h3 className="text-2xl font-bold font-display text-green-400 mb-4">Your Music Video is Ready!</h3>
                    ...
                    <>
                        <h3 className="text-2xl font-bold font-display text-[var(--accent-primary)] mb-4">Ready to Assemble?</h3>
                        <p className="text-[var(--text-secondary)] mb-6">Combine the generated clips, images, and your audio into a final music video.</p>
                        <button
                            onClick={onCombine}
            ...
            <div key={scene.scene} className="group">
                <SectionCard className="h-full transition-all duration-500 group-hover:shadow-[0_0_20px_var(--accent-primary)] group-hover:border-[var(--accent-primary)]">
                  <div className="p-6 h-full flex flex-col">
                    <h3 className="text-xl font-bold font-display text-[var(--accent-primary)]">{sceneTitle}</h3>
                    <p className="text-[var(--text-secondary)] mt-2 text-sm italic">"{scene.description}"</p>
                    <p className="text-xs text-gray-500 mt-2 font-mono bg-black/20 p-2 rounded-md">Prompt: {scene.prompt}</p>

                    {/* Media preview area */}
                    <div className="mt-4 flex-grow flex items-center justify-center bg-black/30 rounded-lg aspect-video">
                      {media?.status === 'generating' && <Loader message="Generating..." />}
                      {media?.status === 'complete' && media.url && (
                        media.type === 'video' ? <VideoPlayer src={media.url} /> : <img src={media.url} alt={`Generated image for scene ${media.scene}`} className="w-full h-full object-cover rounded-lg" />
                      )}
                      {media?.status === 'pending' && <p className="text-gray-400">{statusMessages.pending}</p>}
                      {media?.status === 'failed' && (
                        <div className="text-red-400 text-center p-2">
                            <p className="font-bold">{statusMessages.failed}</p>
                            {media.error && <p className="text-xs mt-1 font-mono bg-red-900/20 p-2 rounded">{media.error}</p>}
                        </div>
                       )}
                    </div>
                     ...
                     <div className="mt-4 text-center text-sm flex items-center justify-center gap-2">
                        <p className={`capitalize font-semibold ${
                          media?.status === 'complete' ? 'text-green-400' : 
                          media?.status === 'generating' ? 'text-yellow-400 animate-pulse' :
                          media?.status === 'failed' ? 'text-red-400' : 'text-gray-400'
                        }`}>
                          Status: {media?.status}
                        </p>
                        {(media?.status === 'complete' || media?.status === 'failed') && (
                            <button 
