...
export const DJ: React.FC = () => {
    const { isTtsErrorMuted, isTtsUserMuted, currentDj } = useContext(RadioContext);
    const isMuted = isTtsErrorMuted || isTtsUserMuted;

    return (
        <div className={`flex items-center gap-2 p-3 bg-black/40 backdrop-blur-md rounded-lg border ${isMuted ? 'border-gray-500/30' : 'border-yellow-400/20'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 flex-shrink-0 ${isMuted ? 'text-gray-400' : 'text-yellow-400'}`} viewBox="0 0 20 20" fill="currentColor">
                {isMuted ? (
            ...
            <div>
                <h3 className={`font-bold text-md font-display leading-tight ${isMuted ? 'text-gray-300' : 'text-yellow-400'}`}>{currentDj.name}</h3>
                <p className="text-xs text-gray-300 leading-tight">{isMuted ? 'Text Only' : 'On Air'}</p>
            </div>
        </div>
    );
};
