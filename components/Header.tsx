...
export const Header: React.FC<HeaderProps> = ({ onNavigate, onSignOut, profile }) => {
  ...
  return (
    <header className="relative flex flex-col w-full pointer-events-none p-2 sm:p-4 gap-3 sm:gap-4">
      ...
      {/* BOTTOM ROW (Mobile only) or SIDE (Desktop): Controls */}
      <div className="flex items-center gap-2 sm:gap-4 pointer-events-auto w-full">
        {/* Desktop Only: Now Playing in Header (Moved from right extreme) */}
        {nowPlaying && (
          <div className="hidden lg:flex items-center gap-3 border-r border-white/5 pr-4 opacity-60 hover:opacity-100 transition-opacity cursor-default shrink-0">
        ...
        <div className="hidden sm:flex items-center gap-3 px-3 py-1 bg-white/5 rounded-full border border-white/5 hover:border-white/10 transition-all flex-grow max-w-[200px]">
          ...
          <div className="w-full h-1 bg-zinc-800 rounded-full relative group/vol overflow-hidden">
            <input
              type="range"
              min="0"
              max="1"
