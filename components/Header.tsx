...
export const Header: React.FC<HeaderProps> = ({ onNavigate, onToggleChat, isChatVisible }) => {
    return (
        <header className="flex justify-between items-center px-4 sm:px-8 py-4 flex-shrink-0">
            <div>
                ...
                <p className="text-sm text-[var(--text-secondary)]">24/7 AI-Powered Radio</p>
                <p className="mt-1 text-xs text-yellow-400/60 italic hidden sm:block">"The Youniverse observing itself listening to its own song"</p>
            </div>
            <div className="flex items-center gap-4">
                ...
                >
                    {isChatVisible ? (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                    ) : (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 12h16M4 12l4-4m-4 4l4 4" /></svg>
                    )}
                </button>
