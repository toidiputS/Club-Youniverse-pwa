  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-4 flex-shrink-0">
        <h3 className="text-xl font-bold font-display text-yellow-400">
          Live Chat
        </h3>
        <button
      ...
      <div className="chat-messages-container flex-grow space-y-3 pr-2 overflow-y-auto mb-4 min-h-0">
        {chatMessages.map((msg) => (
          <div
            key={msg.id}
            className="chat-message rounded-lg p-2 bg-black/50 backdrop-blur-sm border border-white/10"
          >
            <p className="text-gray-200 break-words text-sm">
              <span
                className={`font-semibold mr-2 ${msg.user.name === "Club Youniverse" ? "text-yellow-400" : "text-gray-400"}`}
              >
                {msg.user.name}:
              </span>
              {msg.text}
            ...
            {msg.source && (
              ...
              >
                <span className="font-bold text-yellow-400 block truncate">
                  {msg.source.title}
                </span>
