...
export const UploadScreen: React.FC<UploadScreenProps> = ({
  ...
  const [source, setSource] = useState<
    "suno" | "producer.ai" | "mubert" | "upload"
  >("upload");

  // State for advanced options
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [aspectRatio, setAspectRatio] = useState<SongDetails["aspectRatio"]>(
    settings.defaultAspectRatio,
  );
  const [pacing, setPacing] = useState<"default" | "slow" | "fast">("default");
  const [styleKeywords, setStyleKeywords] = useState(
    settings.defaultStyleKeywords,
  );
  ...
  const handleFileSelect = async (file: File | null) => {
    ...
    if (file) {
      try {
        ...
        setFormError(
          "Could not read audio file duration. Please try a different file.",
        );
        ...
        <header className="flex justify-between items-start mb-12">
          <div className="text-left">
            ...
            <p className="mt-2 text-[var(--text-secondary)]">
              Only VIPs can access AI production tools.
            </p>
          </div>
          <button
            onClick={onBack}
          ...
          <div className="space-y-2">
            ...
            {audioDuration && (
              <p className="text-xs text-green-400 mt-1">
                Duration detected: {audioDuration.toFixed(1)}s
              </p>
            )}
          </div>
              ...
              <div className="flex justify-around text-center">
                <div>
                  ...
                  <p className="font-mono text-white">
                    {formatCurrency(estimatedCost.breakdown.storyboardCost)}
                  </p>
                </div>
                <div>
                  <p className="text-gray-400">
                    {estimatedCost.breakdown.numVideos} Video Clips
                  </p>
                  <p className="font-mono text-white">
                    {formatCurrency(estimatedCost.breakdown.videoCost)}
                  </p>
                </div>
                <div>
                  <p className="text-gray-400">
                    {estimatedCost.breakdown.numImages} Images
                  </p>
                  <p className="font-mono text-white">
                    {formatCurrency(estimatedCost.breakdown.imageCost)}
                  </p>
                </div>
              </div>
              <p className="text-xs text-gray-500 text-center mt-2">
                This is an estimate. Actual cost may vary based on model usage
                and final scene count.
              </p>
            </div>
          )}
