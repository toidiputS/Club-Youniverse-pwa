...
export const Ticker: React.FC = () => {
    const { nowPlaying, djQueue, radioState, boxRound, voteCounts } = useContext(RadioContext);
    ...
    const statusText = useMemo(() => {
        ...
        if (radioState === 'BOX_VOTING' && boxRound) {
            const voteStrings = boxRound.candidates.map(c => 
                `${c.title}: ${(voteCounts[c.id] || 0).toLocaleString()}`
            );
