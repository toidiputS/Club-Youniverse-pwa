...
export const PremiumUpgrade: React.FC<PremiumUpgradeProps> = ({ profile, onUpgradeComplete, onCancel }) => {
    const [phoneNumber, setPhoneNumber] = useState(profile.phone_number || '');
    const [roastConsent, setRoastConsent] = useState(profile.roast_consent || false);
        ...
        <SectionCard>
            <div className="p-8 max-w-lg mx-auto">
                <div className="text-center mb-8">
                    <h2 className="text-3xl font-bold text-[var(--accent-primary)] mb-2 font-display">Upgrade to VIP</h2>
                    <p className="text-[var(--text-secondary)]">Unlock uploads, downloads, and join the elite rotation.</p>
                </div>
                ...
                {isLoading ? (
                    ...
                    <form onSubmit={handleUpgrade} className="space-y-6 text-left">
                        <div className="bg-[var(--accent-secondary)]/10 border border-[var(--accent-secondary)]/30 p-4 rounded-lg mb-6">
                            <h3 className="text-[var(--accent-secondary)] font-bold mb-2">🔥 VIP Perks:</h3>
                        ...
                        <div>
                            <label htmlFor="phone" className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Phone Number (Mandatory for VIP)</label>
                            <input
                                id="phone"
                                type="tel"
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">Used for live interaction with the AI DJ.</p>
                        </div>
                        ...
                        <div className="flex items-start gap-3 p-4 bg-red-900/10 border border-red-500/20 rounded-lg">
                            ...
                            />
                            <label htmlFor="roast-consent" className="text-sm text-[var(--text-primary)] leading-tight">
                                <span className="font-bold text-red-400 block mb-1">LIVE ROAST DISCLOSURE</span>
                                I agree to be roasted live on air by the AI DJ if my song hits zero stars. I understand this involves a live call to the phone number provided above.
                            </label>
                        </div>
