...
export const Leaderboard: React.FC<LeaderboardProps> = ({ onBackToStudio }) => {
  const [songs, setSongs] = useState<Song[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [sortBy, setSortBy] = useState<SortByType>("stars"); // Default sort by stars

  // Effect to fetch and sort the songs.
  useEffect(() => {
      ...
      return (
        <SectionCard>
          <div className="p-8 text-center text-gray-400">
            The leaderboard is empty. Submit some songs and see how they rank!
          </div>
        </SectionCard>
      );
    }
      ...
      <header className="flex justify-between items-start mb-4">
        <div className="text-left">
          ...
          <p className="mt-2 text-[var(--text-secondary)]">
            The hottest tracks on Club Youniverse Live.
          </p>
        </div>
        <button
