...
export const Profile: React.FC<ProfileProps> = ({
  ...
  const userCreations = galleryItems.filter(
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <header className="flex justify-between items-start mb-4">
        <div className="text-left">
          ...
          <p className="mt-2 text-[var(--text-secondary)]">
            Manage your creator identity and app preferences.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <SectionCard>
          <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
              <h2 className="text-2xl font-bold font-display text-[var(--accent-primary)]">
                Creator Profile
              </h2>
              {profile.is_premium && (
                <span className="animate-pulse bg-gradient-to-r from-yellow-400 to-amber-600 text-white text-xs font-black px-3 py-1 rounded-full shadow-[0_0_15px_rgba(245,158,11,0.6)] border border-yellow-300">
                  👑 VIP MEMBER
                </span>
              )}
            </div>
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              ...
              <div>
                ...
                {!profile.name && (
                  <p className="text-xs text-yellow-400 mt-1">
                    Please set your artist name to continue.
                  </p>
                )}
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[var(--accent-secondary)] text-white font-bold py-3 px-6 rounded-lg hover:bg-[var(--accent-primary)] transition-colors disabled:bg-gray-500"
              >
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gray-700 text-white font-bold py-3 px-6 rounded-lg hover:bg-gray-600 transition-colors"
            >
              Sign Out
            </button>

            {/* Hidden Admin/Dev Tool for VIP Grant */}
            {session.user.email === "itstraderbaby@gmail.com" &&
              !profile.is_premium && (
                <div className="mt-4 border-t border-[var(--input-border)] pt-4">
                  <p className="text-xs text-gray-500 mb-2">
                    Dev Tools (Visible only to you)
                  </p>
                  <button
                    onClick={async () => {
                      setLoading(true);
                      ...
                      if (error) {
                        ...
                        if (
                          ...
                          setError(
                            "Schema Error: The 'is_premium' column is missing in your Supabase database. Please run the 'SUPABASE_FIX_PREMIUM.sql' script in your Supabase SQL Editor to fix this.",
                          );
                        } else {
        ...
        <SectionCard>
          <div className="p-8">
            <h2 className="text-2xl font-bold font-display text-[var(--accent-primary)] mb-6">
              App Settings
            </h2>
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleSettingsChange({ theme: e.target.value as ThemeName })
                  }
                  className={inputStyles}
                >
              ...
              <div>
                <label
                  htmlFor="styleKeywords"
                  className="block text-sm font-medium text-[var(--text-secondary)] mb-1"
                >
                  Default Video Style Keywords
                </label>
                <input
                  id="styleKeywords"
                  type="text"
                  value={settings.defaultStyleKeywords}
                  onChange={(e) =>
                    handleSettingsChange({
                      defaultStyleKeywords: e.target.value,
                    })
                  }
                  className={inputStyles}
                />
      ...
      <div>
        <h2 className="text-3xl font-bold font-display text-[var(--accent-primary)] mb-4">
          My Creations
        </h2>
