...
const COST_PER_VEO_VIDEO_CLIP = 0.10;      // Example cost per generated video clip
...
): { total: number; breakdown: { storyboardCost: number; videoCost: number; imageCost: number; numVideos: number; numImages: number; } } => {
    if (!audioDuration || audioDuration <= 0) {
        return { total: 0, breakdown: { storyboardCost: 0, videoCost: 0, imageCost: 0, numVideos: 0, numImages: 0 } };
    }

    // 1. Calculate the number of scenes based on pacing (logic duplicated from geminiService).
    const numberOfScenes = pacing === 'slow' 
