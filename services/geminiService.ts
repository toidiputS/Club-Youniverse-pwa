...
const getApiKey = (): string | undefined => {
  ...
  return viteEnv.VITE_API_KEY || (typeof process !== 'undefined' && process.env && process.env.API_KEY);
};
...
const getAiClient = () => {
    ...
    if (!apiKey) {
        throw new Error("API_KEY environment variable not set. Please select or provide an API key.");
    }
...
export const analyzeSongAndGenerateStoryboard = async (
    ...
    const numberOfScenes = pacing === 'slow' ? Math.max(1, Math.floor(audioDuration / 15)) : pacing === 'fast' ? Math.floor(audioDuration / 5) : Math.floor(audioDuration / 10);
    ...
    try {
        ...
        throw new Error("Failed to generate storyboard from AI. The model may have returned an invalid format or an error occurred.");
    }
};
...
export const generateVideoClip = async (
    ...
    try {
        ...
        const apiKey = getApiKey();
        if (!apiKey) {
             throw new Error("API key is missing when trying to fetch generated video.");
        }
        ...
        if (e.message?.includes("Requested entity was not found")) {
            throw new Error("The selected API key is not valid for Veo or project setup is incomplete. Please select a different key and check your billing setup.");
        }
 ...
 * @throws An error if image generation fails.
 */
export const generateAlbumCover = async (prompt: string): Promise<{ url: string }> => {
    const ai = getAiClient();
    try {
        const response = await ai.models.generateImages({
            model: 'imagen-4.0-generate-001',
...
export const generateSpeech = async (script: string, voiceName: string): Promise<string> => {
    ...
    try {
        ...
        if (e.message?.includes("API key not valid") || e.message?.includes("403")) {
             throw new Error("The selected API key is not valid for TTS or has been disabled. Please select a different key.");
        }
        // Specific check for rate-limiting (429 Too Many Requests)
        if (e.message?.includes("429") || e.message?.includes("RESOURCE_EXHAUSTED")) {
             throw new Error("TTS API rate limit exceeded. Please wait a moment before trying again. (429)");
        }
