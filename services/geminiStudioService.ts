...
const getNextApiKey = (): string => {
    ...
    if (keys.length === 0) {
        throw new Error("No API keys available. Please add VITE_API_KEY to .env.local");
    }
...
const selectBoxCandidatesTool: FunctionDeclaration = {
    ...
    description: "Given a pool of songs, select three candidates with weighted randomness and novelty.",
    parameters: {
        type: Type.OBJECT,
        properties: {
            candidateIds: {
...
const djBanterScriptTool: FunctionDeclaration = {
    ...
    parameters: {
        type: Type.OBJECT,
        properties: {
            script: { type: Type.STRING, description: "Clean, on-air friendly lines for the DJ." },
            cta: { type: Type.STRING, description: "An optional call-to-action for voting or premium features." },
            safety: {
                type: Type.OBJECT,
                properties: {
...
const coverLayoutPlanTool: FunctionDeclaration = {
    ...
    parameters: {
        type: Type.OBJECT,
        properties: {
            front: { type: Type.OBJECT, properties: { prompt: { type: Type.STRING }, safe: { type: Type.BOOLEAN } }, required: ['prompt', 'safe'] },
            back: { type: Type.OBJECT, properties: { prompt: { type: Type.STRING }, safe: { type: Type.BOOLEAN } }, required: ['prompt', 'safe'] },
            spine: { type: Type.OBJECT, properties: { text: { type: Type.STRING } }, required: ['text'] },
            inside: {
                type: Type.ARRAY,
                items: {
                    type: Type.OBJECT,
...
const videoStylePlanTool: FunctionDeclaration = {
    ...
    description: "Plan a music video from the song's mood/tempo; feeds your existing generator.",
    parameters: {
        type: Type.OBJECT,
        properties: {
            scenes: {
 ...
 */
export const selectBoxCandidates = async (input: SelectBoxCandidatesInput): Promise<SelectBoxCandidatesOutput> => {
    const ai = getAiClient();
    const prompt = `
    ...
    const response = await ai.models.generateContent({
        ...
        contents: prompt,
        config: {
            ...getModelConfig("You are the station's music director, an impartial AI responsible for selecting songs for The Box based on data, not preference."),
    ...
    throw new Error("Gemini did not return the expected function call for select_box_candidates.");
};
 ...
 */
export const generateDjBanter = async (input: DjBanterScriptInput): Promise<DjQueueItem | null> => {
 ...
 */
export const generateZeroStarCallCopy = async (input: ZeroStarCallCopyInput): Promise<ZeroStarCallCopyOutput> => {
    const ai = getAiClient();
    const response = await ai.models.generateContent({
        model: 'gemini-2.5-pro',
    ...
    throw new Error("Gemini did not return the expected function call for zero_star_call_copy.");
};
...
export const cleanLyrics = async (input: LyricsDetectAndCleanInput): Promise<LyricsDetectAndCleanOutput> => {
    const ai = getAiClient();
    const response = await ai.models.generateContent({
        model: 'gemini-2.5-pro',
    ...
    throw new Error("Gemini did not return the expected function call for lyrics_detect_and_clean.");
};

export const generateCoverLayoutPlan = async (input: CoverLayoutPlanInput): Promise<CoverLayoutPlanOutput> => {
    const ai = getAiClient();
    const prompt = `
    ...
    throw new Error("Gemini did not return the expected function call for cover_layout_plan.");
};

export const generateVideoStylePlan = async (input: VideoStylePlanInput): Promise<VideoStylePlanOutput> => {
    const ai = getAiClient();
    const prompt = `
    ...
    throw new Error("Gemini did not return the expected function call for video_style_plan.");
};