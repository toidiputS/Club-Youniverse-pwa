...
async function cachePremiumCallouts(): Promise<void> {
    ...
    if (error) {
        console.error('Error fetching premium callouts from Supabase Storage:', error);
    ...
    premiumCalloutsCache = data.filter(file => file.name !== '.emptyFolderPlaceholder');
}
...
export async function getRandomPremiumCalloutUrl(): Promise<string | null> {
    ...
    if (!premiumCalloutsCache || premiumCalloutsCache.length === 0) {
        console.warn('No premium callouts found in Supabase Storage or cache is empty.');
        return null;
    }
