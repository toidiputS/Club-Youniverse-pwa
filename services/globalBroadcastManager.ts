...
export class GlobalBroadcastManager {
    ...
    private startElectionLoop() {
        ...
        // Check for leadership every 5 seconds
    ...
    private async tryClaimLeadership() {
        try {
            ...
            const now = new Date();
            const heartbeat = data.last_heartbeat ? new Date(data.last_heartbeat) : new Date(0);
            const secondsSinceHeartbeat = (now.getTime() - heartbeat.getTime()) / 1000;
            const isCurrentLeaderDead = secondsSinceHeartbeat > 15; // 15s timeout
            ...
            if (amILeader) {
                ...
                console.log(`👑 Leader missing (${secondsSinceHeartbeat.toFixed(1)}s ago). Claiming throne...`);
    ...
    private async initializeGlobalState() {
        ...
        supabase
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'broadcasts' }, _payload => {
    ...
    private syncStateFromRemote(data: any) {
        ...
        if (this.state.radioState !== remoteState) {
            console.log(`📻 Global State Update: ${this.state.radioState} -> ${remoteState}`);
    ...
    private saveState(): void {
        try {
            ...
            localStorage.setItem('club-youniverse-broadcast-state', JSON.stringify(toSave));
        } catch (error) {
    ...
    public async setNowPlaying(song: Song | null, startOffset: number = 0): Promise<void> {
        ...
        const isSameSong = this.audioElement.src === song.audioUrl || (this.state.nowPlaying?.id === song.id);
        ...
        if (!isSameSong) {
            ...
            if (startOffset > 0 && Math.abs(this.audioElement.currentTime - startOffset) > 2) {
                this.audioElement.currentTime = startOffset;
            }
        }
    ...
    public getCurrentTime() { return this.audioElement.currentTime; }
