...
async function verifySongs() {
    ...
    console.log(`   In Box: ${songs.filter(s => s.status === 'in_box').length}`);
    console.log(`   Playing: ${songs.filter(s => s.status === 'now_playing').length}`);
    console.log(`   Graveyard: ${songs.filter(s => s.status === 'graveyard').length}`);
}
